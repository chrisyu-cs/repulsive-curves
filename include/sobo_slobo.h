...
namespace LWS {
    ...
    class SobolevCurves {
        ...
        static void ApplyDf(PolyCurveGroup* loop, Eigen::VectorXd &as, Eigen::MatrixXd &out);
        // Multiply by the transpose of the above Df (taking edge values back to vertices)
        static void ApplyDfTranspose(PolyCurveGroup* loop, Eigen::MatrixXd &es, Eigen::VectorXd &out);
